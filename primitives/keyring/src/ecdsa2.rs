...
pub use sp_core::ecdsa2;
use sp_core::{
    ecdsa2::{Pair2, Public2, Signature2},
...
impl Keyring {
    pub fn from_public(who: &Public2) -> Option<Keyring> {
        Self::iter().find(|&k| &Public2::from(k) == who)
    }
    ...
    //     Self::from_public(&Public2::from_raw(who))
    // }
    //
    // pub fn to_raw_public(self) -> [u8; 32] {
    //     *Public2::from(self).as_array_ref()
    // }
    ...
    pub fn from_h256_public(who: H256) -> Option<Keyring> {
        Self::from_public(&Public2::from_raw(who.into()))
    }

    pub fn to_h256_public(self) -> H256 {
        Public2::from(self).as_array_ref().into()
    }

    pub fn to_raw_public_vec(self) -> Vec<u8> {
        Public2::from(self).to_raw_vec()
    }
    ...
    pub fn sign(self, msg: &[u8]) -> Signature2 {
        Pair::from(self).sign(msg)
    }

    pub fn pair(self) -> Pair2 {
        Pair2::from_string(&format!("//{}", <&'static str>::from(self)), None)
...
lazy_static! {
	...
	static ref PUBLIC_KEYS: HashMap<Keyring, Public2> =
		PRIVATE_KEYS.iter().map(|(&name, pair)| (name, pair.public())).collect();
}

impl From<Keyring> for Public2 {
    fn from(k: Keyring) -> Self {
...
impl AsRef<Public2> for Keyring {
    fn as_ref(&self) -> &Public2 {
